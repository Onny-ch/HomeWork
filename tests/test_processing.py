...
def test_sort_by_date_reverse(account_transaction_data):
    assert sort_by_date(account_transaction_data, True) == sorted(
        account_transaction_data, key=lambda transact_data_dict: transact_data_dict["date"], reverse=True
    )


def test_sort_by_date_not_reverse(account_transaction_data):
    assert sort_by_date(account_transaction_data, False) == sorted(
        account_transaction_data, key=lambda transact_data_dict: transact_data_dict["date"], reverse=False
    )
